 ...
 Ext.define('GeoExt.component.FeatureRenderer', {
     ...
     initComponent: function () {
        ...
        me.on('afterrender', function () {
            ...
            me.nativeCtx = me.canvas.getContext('2d');
            me.vectorContext = ol.render.toContext(me.canvas.getContext('2d'), {size: [120, 120]});
    ...
    drawFeature: function () {
        ...
        if (!me.symbolizers) {
            ...
            //TODO detect style from geom or layer
            return;
        } else {
            console.log("symb", me.symbolizers);
            me.vectorContext.setStyle(me.symbolizers);
        }

        me.nativeCtx.translate(60, 60);

        // me.vectorContext.drawGeometry(new ol.geom.LineString([[10, 10], [90, 90]]));
        // me.vectorContext.drawGeometry(new ol.geom.Polygon([[[2, 2], [98, 2], [2, 98], [2, 2]]]));
        // var g = new ol.geom.Point([88, 88]);
        // var g = new ol.geom.LineString([[897676,6371790], [1181410,6880555], [1362413,6269059]]);
        var g = new ol.geom.Polygon([[[2, 2], [98, 2], [2, 98], [2, 2]]]);

        if (!me.feature) {
            return;
        }
        // var gc = g.clone();
        var gc = g.clone().transform('EPSG:3857', 'EPSG:4326');
        // var gc = me.feature.getGeometry().clone();

        var extWidth = ol.extent.getWidth(gc.getExtent());
        var extHeight = ol.extent.getHeight(gc.getExtent());

        console.log("ex w h", extWidth, extHeight);

        var scaleW = 120 *0.9 / extWidth;
        var scaleH = 110 *0.9 / extHeight;

        console.log(scaleW, scaleH);

        var minScale = Math.min(scaleW, -scaleH);

        gc.scale(minScale, minScale);

        me.vectorContext.drawGeometry(gc);
    }
